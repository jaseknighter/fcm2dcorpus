...
function gc:init()
  ...
  function transport_keys.key(x,y,z)  -- define what happens if a grid key is pressed or released
    ...
    if points_data and transport_key_exists and z == 1 then 
      ...
      local retrigger = 1
      osc.send( { "localhost", 57120 }, "/sc_fcm2dcorpus/play_slice",{p_x,p_y,params:get("slice_volume"),retrigger})
  ...
  function transport_keys:set_pages(
    ...
    function transport_keys.pages.key(x,y,z)
      local page_id = transport_keys.pages.ids[x][y]
      if page_id and page_id ~= self.current_page and z == 1 then
        print("transport pages key id ", page_id, " at " .. x,y)
        self.current_page = page_id
        print("new current page",self.current_page)
        local total_keys = keys_per_row*keys_per_col
        local new_starting_key = 1+(page_id-1)*total_keys
        self:set_area(
